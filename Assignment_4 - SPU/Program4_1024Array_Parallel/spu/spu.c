/*
In order to parallelize we need to make sure that local memory storage on each SPU is within limits.
int MatrixSPE[16][1024] (65536 bytes)
int TransposeSPE[16][1024] (65536 bytes)
int MultSPE[16][16] (1024 bytes)
...
int MatrixSPE[16][1024] QWORD_ALIGNED;
int TransposeSPE[16][1024] QWORD_ALIGNED;
